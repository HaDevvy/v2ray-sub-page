...
نکته: بهتر است `config/config.json` را JSON معتبر نگه داری و داخلش از `#` برای کامنت استفاده نکنی. با این حال loader جدید برای جلوگیری از بی‌اثر شدن کل config، کامنت‌های `#`، `//`، `/* ... */` و trailing comma را هم قبل از parse حذف می‌کند.
